...
class TestPartitionFilesShowKnown(unittest.TestCase):
    ...
    def _setup_partition(self):
        ...
        with self.app.app_context():
            ...
            # A file is "known" when it is linked to a KnownFile row
            # (is_known boolean was dropped in favour of the known_file_id FK).
            db = HashDatabase(name=f'test-db-{uuid.uuid4().hex[:8]}')
            self.db.session.add(db)
            self.db.session.flush()
            kf = KnownFile(database_id=db.id, filename='known.txt',
                           md5='aaaa1111bbbb2222cccc3333dddd4444', file_size=100)
            self.db.session.add(kf)
            self.db.session.flush()
            ef_known.known_file_id = kf.id
            self.db.session.commit()
