...
class AnalysisWorker:
    ...
    def queue_partition_follow_ups(
        ...
        *categories* is the set of :class:`ContentCategory` actually present in
        ...
        # NSFW scan — gated on CONVERTIBLE (the same raster/image files the
        # classifier reads); only when there is an extraction tree to scan.
        if extraction_path and present(ContentCategory.CONVERTIBLE):
            ...
            if NSFW_ENABLED:
                ...
                if path_prefix:
                    nsfw_hints[HintKey.PATH_PREFIX] = path_prefix
                self.api.queue_analysis(
                    artefact_uuid,
