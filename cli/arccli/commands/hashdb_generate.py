...
def collect_canonical_candidates(gathered: list) -> dict[str, list]:
    """Find app-dirs that appear with *differing* content on multiple artefacts.

    Returns ``{app_dir_name: [product_name_per_occurrence, ...]}`` — the
    applications a curator must adjudicate.  Copies that are byte-identical across
    every artefact are excluded: identical-copy merging already collapses them
    into one product, so they need no canonical rule.  App-dir names containing
    whitespace (e.g. synthetic root/disc keys) are skipped as the rule-file
        ...
        for ar in g['artefact_results']:
            ...
            for name, files in ar['app_dirs'].items():
                if any(c.isspace() for c in name):
                    continue
                occ.setdefault(name, []).append((disp, _appdir_fingerprint(files)))
    ...
    for name, entries in occ.items():
        ...
        result[name] = [disp for disp, _fp in entries]
    return result
...
def render_canonical_candidates(candidates: dict[str, list]) -> str:
    ...
    All candidate lines are emitted; a line is left *active* (uncommented) only
    when exactly one artefact's product name unambiguously looks like the golden
    source (its name starts with the app's base name).  When several — or none —
    match, every line is commented out so the curator picks deliberately rather
    than silently keeping the wrong (or multiple) copies.  Likely-golden lines
    are listed first within each app.
    """
    out = [
        ...
        '# Keep ONE rule line per app (or several, for multiple golden versions) and',
        '# comment (#) or delete the rest, then pass this file via',
        '# --canonical-sources.  A line',
        '# is pre-activated only when a single obvious golden source was found;',
        '# otherwise all lines are commented — uncomment the one(s) you want.',
        '#',
    ...
    for name in sorted(candidates):
        ...
        matches = [n for n in uniq if n and guess_re.match(n)]
        # Only trust the guess when it is unambiguous (exactly one match).
        active = matches[0] if len(matches) == 1 else None
        # List the likely-golden candidate(s) first for easy scanning.
        ordered = [n for n in uniq if n in matches] + [n for n in uniq if n not in matches]

        out.append(f'# {name} — appears on {len(occ)} artefact(s):')
        if active is None and matches:
            out.append('#   (multiple possible golden sources — uncomment the right one(s))')
        for n in ordered:
            # Escape regex metacharacters but keep spaces/hyphens readable.
            pat = (re.escape(n).replace('\\ ', ' ').replace('\\-', '-')) if n else '.*'
            prefix = '' if n == active else '#'
...
def merge_identical_products(products: list[dict]) -> tuple[list[dict], int]:
    ...
    order: list = []
    for p in products:
        key = (p.get('_app_dir'), _product_fingerprint(p))
