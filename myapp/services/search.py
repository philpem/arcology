"""
Arcology - Search service

The query engine behind the global search blueprint: the prefix-query parser
(:func:`parse_query`), the per-key sub-searches, and the result-bucket
assembler (:func:`_run_search`).  Kept separate from
``myapp/blueprints/search.py`` so the blueprint stays a thin routes +
presentation layer and the query logic can be unit-tested (and later extended
for full-text search) in isolation.

Visibility is enforced here, not by the caller: every sub-search applies
``artefact_visibility_clause`` / ``item_visibility_clause`` against
``current_user`` so no code path can accidentally return rows the user may not
see.
"""
...
def _search_documents(tokens, page=1, per_page=PER_PAGE):
    ...
    * **SQLite** (tests / lightweight dev) — substring ILIKE on ``content``, no
    ...
    total = _cached_count('documents', tokens, lambda: _count_rows(q))
...
def run_duplicate_search(key: str, page: int = 1, per_page: int = PER_PAGE):
    """Return ``(rows, total)`` for every visible copy of one content hash.

    *key* is the ``COALESCE(sha256, sha1, md5)`` group key produced by the
    collapsed (deduped) file search — the "×N" duplicate badge target.  Results
    are visibility-filtered, so a user only ever sees (and counts) the copies
    they are allowed to see.  Rows are ``(ExtractedFile, Partition, Artefact,
    Item)`` tuples in the canonical file order.
    """
    hashkey = _file_hashkey()
    q = (
        db.session.query(ExtractedFile, Partition, Artefact, Item)
