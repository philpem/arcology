...
- `STALE_JOB_TIMEOUT_SECONDS` (default 3600) controls how long a job may stay in `RUNNING` state before it is considered stuck and eligible for reset back to `PENDING`. Set this above the longest expected analysis run time. The worker re-queues stale jobs both at startup and periodically (`STALE_RESET_INTERVAL`, default 300s, set on the worker) — so a job orphaned by a crash or a SIGKILL past the stop grace period is recovered automatically without waiting for a worker restart. The reset is a full re-run from scratch (all partial result fields are cleared), which is safe because the analysis pipeline is idempotent.
