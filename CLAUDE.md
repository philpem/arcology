...
- **Avoid deprecated/legacy SQLAlchemy APIs.** The app-tests runner (`ci/run_app_tests.py`) escalates `DeprecationWarning` (which includes SQLAlchemy's `LegacyAPIWarning`) to a hard error, so legacy calls that merely warn locally **fail in CI**. Most common offender: `Model.query.get(pk)` — use `db.session.get(Model, pk)` instead. Prefer the 2.0-style `db.session.get(...)` / `db.session.scalars(db.select(...))` over the legacy `Model.query.…` interface in new code. A plain local `python -m unittest` does **not** escalate warnings, so run `python ci/run_app_tests.py` (or escalate warnings manually) to reproduce CI's behaviour before pushing.
...
- **Never load a whole artefact into RAM unless you already know its size and that it fits.** Artefacts are routinely multi-GB (disc/flux images, hard-disc dumps), so `path.read_bytes()` / `f.read()` / `bytearray(...)` over an *unbounded* file is an OOM/DoS waiting to happen — a 6 GB ADFS hard disc OOM-killed a worker via an inline ARMlock probe that did `bytearray(image_path.read_bytes())`. Rules of thumb for worker code:
  - **Random access into a large image** (parsers that index/slice at scattered offsets): use `open_sector_reader(path)` → `SectorReader` (`worker/arcworker/tools/base.py`). It supports `len()`, `buf[i]` and `buf[a:b]` via bounded seek/read; **slice before `struct.unpack_from`** (a `SectorReader` has no buffer protocol). ARMlock detection (`fs_riscos_armlock.py`) is the worked example.
  - **A known, bounded region** (a header, a sector, an embedded sprite): seek to it and read exactly that many bytes, after a sanity-cap check that the length isn't absurd (see `_save_replay_poster` and `parse_armovie_header`'s bounded header/catalogue reads in `metadata.py` / `armovie.py`, and the `max_bytes` cap in `_extract_file_by_sin`).
  - **A whole file that is *inherently* small** (sprite, RISC OS module, IMD floppy, extracted text): use `read_file_capped(path)` (`base.py`), which refuses anything over `MAX_INMEM_BYTES` (256 MiB) by raising `FileTooLargeError` (an `OSError`, so existing `except OSError` handlers degrade cleanly) instead of slurping a misdetected/hostile giant.
  - **In-place edits of a large image** (e.g. ARMlock removal): `shutil.copy()` then patch the affected sectors with seek/read/write on the copy — never read-modify-write the whole file in memory.
  - Covered by `ci/test_worker_io.py` (SectorReader semantics, the cap, and a sparse-image regression test that asserts memory doesn't scale with image size).
- `STALE_JOB_TIMEOUT_SECONDS` (default 3600) controls how long a job may go **without any sign of life** before it is considered stuck and eligible for reset back to `PENDING`. Staleness is heartbeat-based: a `RUNNING` job is stale only when `COALESCE(Analysis.progress_updated_at, Analysis.started_at) < now − timeout` (see `reset_stale_analyses` in `myapp/blueprints/api.py` and `reset_stale`/`_stale_cutoff` in `myapp/blueprints/analysis.py`). The worker's per-job cancellation-monitor thread heartbeats every `CANCEL_CHECK_INTERVAL` (30s) and handlers report item progress via `self.progress` (`ProgressReporter`), both of which bump `progress_updated_at` — so a legitimately long job stays alive and you can set the timeout to "max acceptable silence" (must stay comfortably above `CANCEL_CHECK_INTERVAL`) rather than "longest expected run". The monitor heartbeat is capped at `HEARTBEAT_MAX_SECONDS` (default 6h, set on the worker): it only proves the *process* is alive, so past the cap a handler wedged on an unbounded call (a timeout-less network/storage read, an infinite loop) stops being heartbeated and becomes eligible for reset — a genuinely progressing handler bumps `progress_updated_at` itself and is unaffected. The worker re-queues stale jobs both at startup and periodically (`STALE_RESET_INTERVAL`, default 300s, set on the worker) — so a job orphaned by a crash or a SIGKILL past the stop grace period is recovered automatically without waiting for a worker restart. The reset is a full re-run from scratch (all partial result and progress fields are cleared), which is safe because the analysis pipeline is idempotent.
...
- **Jinja2 `{% block %}` inside `{% if %}` is unreliable.** Block tags are resolved at parse/compile time for template inheritance, but `{% if %}` is evaluated at render time. Wrapping a `{% block %}` in `{% if %}` works in simple cases but breaks subtly when a child template tries to override the conditional block, or when the condition affects whether `{{ super() }}` is called. Always put the `{% if %}` *inside* the block, never wrap the block in the conditional:
  ```jinja
  {# Wrong — block definition is conditional: #}
  {% if condition %}{% block scripts %}…{% endblock %}{% endif %}
  
  {# Right — block always defined, guard is inside: #}
